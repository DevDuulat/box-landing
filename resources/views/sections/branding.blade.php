<div id="branding" class="relative bg-white py-12 md:py-24 px-4 md:px-8 overflow-hidden" x-data="{
    activeSlide: 0,
    slides: [
    ...
    <div class="relative z-10 max-w-4xl mx-auto flex flex-col items-center">
        ...
        <div class="relative w-full max-w-2xl flex items-center justify-center mb-8 md:mb-12">
            <button @click="prev()" class="absolute left-0 md:left-[-100px] p-2 text-gray-800 hover:text-orange-500 transition-colors z-20 bg-white/80 md:bg-transparent rounded-full shadow-sm md:shadow-none">
            ...
            <div class="w-full overflow-hidden rounded-xl md:rounded-2xl shadow-sm border border-gray-100 bg-white">
                <template x-for="(slide, index) in slides" :key="index">
                    <img x-show="activeSlide === index"
                         ...
                         class="w-full aspect-[3/2] object-cover mx-auto">
                </template>
            </div>

            <button @click="next()" class="absolute right-0 md:right-[-100px] p-2 text-gray-800 hover:text-orange-500 transition-colors z-20 bg-white/80 md:bg-transparent rounded-full shadow-sm md:shadow-none">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-8 h-8 md:w-12 md:h-12">
        ...
        <div class="text-center max-w-3xl space-y-4 md:space-y-6">
            ...
            <div class="flex flex-col items-center text-gray-600 space-y-3 px-2">
                <span class="flex items-start gap-2 italic text-sm md:text-base text-center md:text-left">
                    <span class="text-orange-500 leading-none">•</span> {{ __('messages.branding.service_1') }}
                </span>
                <span class="flex items-start gap-2 italic text-sm md:text-base text-center">
                    <span class="text-orange-500 leading-none">•</span> {{ __('messages.branding.service_2') }}
                </span>
            </div>

            <p class="text-gray-800 font-bold text-lg md:text-xl pt-2 md:pt-4 px-4">
